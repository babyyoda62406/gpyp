...
export class UserPrivilegesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean | Promise<boolean> | Observable<boolean> {
    ...
    for(const privileges of requirePrivileges) {
      if(user.privileges.includes(privileges)) return true;
    }
    
    throw new HttpException({message:`User with email: ${user.email} need some of the following privileges: ${requirePrivileges}`}, HttpStatus.FORBIDDEN);
  }
}
